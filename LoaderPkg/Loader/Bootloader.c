  ...
  )
{
  ...
  for (; i < GraphicsOutput->Mode->MaxMode; i++) {
      ...
      if (EFI_ERROR(Status)) {
          DEBUG((DEBUG_ERROR, "JOS: Failed to obtain Graphics Output Mode Info - %r\n", Status));
          return Status;
      }
      DEBUG((DEBUG_INFO, "Mode: %u, Horizontal: %u, Vertical: %u\n", i, Info->HorizontalResolution, Info->VerticalResolution));
  }
  ...
  if (EFI_ERROR(Status)) {
      DEBUG((DEBUG_ERROR, "JOS: Failed to set Graphics Output Mode - %r\n", Status));
      return Status;
  }
