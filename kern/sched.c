...
sched_yield(void) {
    ...
    envid_t cur_id = curenv ? ENVX(curenv->env_id) : 0;
    envid_t start_id = cur_id;
    for (;cur_id < NENV; ++cur_id) {
        if (envs[cur_id].env_status == ENV_RUNNABLE) {
            env_run(envs + cur_id);
        }
    }
    for (cur_id = 0; cur_id < start_id; ++cur_id) {
        if (envs[cur_id].env_status == ENV_RUNNABLE) {
            env_run(envs + cur_id);
        }
    }
    if (envs[start_id].env_status == ENV_RUNNING) {
        env_run(envs + start_id);
    }
