...
env_init(void) {
    ...
    env_free_list = envs;
    struct Env *tmp_envs = envs;
    struct Env *last_envs = &envs[NENV - 1];
    while (tmp_envs != last_envs) {
        tmp_envs->env_status = ENV_FREE;
        tmp_envs->env_link = &tmp_envs[1];
        tmp_envs->env_id = 0;
        tmp_envs++;
    }
    tmp_envs->env_status = ENV_FREE;
    tmp_envs->env_link = NULL;
    tmp_envs->env_id = 0;
}
...
env_alloc(struct Env **newenv_store, envid_t parent_id, enum EnvType type) {
    ...
    env->env_tf.tf_rsp = stack_top - ENVX(env->env_id) * PROG_STACK_SIZE;
...
bind_functions(struct Env *env, uint8_t *binary, size_t size, uintptr_t image_start, uintptr_t image_end) {
    ...
    const char *sh_name_tab = (char *)binary + sh[eh->e_shstrndx].sh_offset;

    size_t strtab_idx = -1;
    for (size_t i = 0; i < eh->e_shnum; ++i) {
        if (sh[i].sh_type == ELF_SHT_STRTAB && !strcmp(".strtab", sh_name_tab + sh[i].sh_name)) {
            strtab_idx = i;
            break;
        }
    }
    ...
    const char *sym_name_tab = (char *)binary + sh[strtab_idx].sh_offset;
    struct Elf64_Sym *symbols = (struct Elf64_Sym *)(binary + sh[symtab_idx].sh_offset);

    size_t n = sh[symtab_idx].sh_size / sizeof(*symbols);
    for (size_t i = 0; i < n; ++i) {
        if (ELF_ST_TYPE(symbols[i].st_info) == STT_OBJECT && ELF_ST_BIND(symbols[i].st_info) == STB_GLOBAL && symbols[i].st_size == sizeof(void *)) {
            if (image_start < (uintptr_t)&(symbols[i].st_value) && (uintptr_t)&(symbols[i].st_value) < image_end) {
                uintptr_t func_addr = find_function(sym_name_tab + symbols[i].st_name);
                if (func_addr) {
                    memcpy((void *)symbols[i].st_value, &func_addr, sizeof(func_addr));
                }
            }
        }
    }
